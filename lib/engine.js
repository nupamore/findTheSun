  ...
  set: function () {
    ...
    if(Camera.target.state.match('rightwalk')){
      ...
      for(var i in allSprites){
        ...
        allSprites[i].x -= Math.pow((d/300),3)*(Camera.target.moveSpeed/10);
      }//for
    }//if right
...
this.Sprite = function(callback){
  ...
  my.animate = function(src, x, y, s, frame){
    ...
    my.ani.interval = setInterval(function(){
  ...
  my.setInterval = function(interval){
    my.intervals.push(setInterval(interval));
    return my;
  };
